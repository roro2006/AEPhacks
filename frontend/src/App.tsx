  ...
  return (
    <div className="app-container">
        ...
        {activeTab === "table" && (
          <div className="data-table-view">
            ...
            {!loading && ratings && (() => {
              ...
              switch (filter) {
                case "critical":
                  filteredLines = filteredLines.filter(l => l.loading_pct >= 100);
                  break;
                case "high":
                  filteredLines = filteredLines.filter(l => l.loading_pct >= 90 && l.loading_pct < 100);
                  break;
                case "caution":
                  filteredLines = filteredLines.filter(l => l.loading_pct >= 60 && l.loading_pct < 90);
                  break;
                case "normal":
                  filteredLines = filteredLines.filter(l => l.loading_pct < 60);
                  break;
                case "az":
      ...
      {selectedLine && ratings && (
        <div className="line-detail-overlay" onClick={() => setSelectedLine(null)}>
          <div className="line-detail-pane" onClick={(e) => e.stopPropagation()}>
            {(() => {
              const lineData = ratings.lines.find(l => l.name === selectedLine);
              ...
              return (
                <>
                  ...
                  <div className="line-detail-content">
                    ...
                    <div style={{
                      padding: "1rem",
                      background: lineData.loading_pct >= 100 ? "rgba(239, 68, 68, 0.1)" :
                                  lineData.loading_pct >= 90 ? "rgba(249, 115, 22, 0.1)" :
                                  lineData.loading_pct >= 60 ? "rgba(234, 179, 8, 0.1)" :
                                  "rgba(16, 185, 129, 0.1)",
                      borderRadius: "12px",
                      border: `2px solid ${lineData.loading_pct >= 100 ? "#ef4444" :
                                           lineData.loading_pct >= 90 ? "#f97316" :
                                           lineData.loading_pct >= 60 ? "#eab308" :
                                           "#10b981"}`,
                      marginBottom: "1.5rem"
                    }}>
                      <div style={{ fontSize: "0.75rem", textTransform: "uppercase", fontWeight: 600, marginBottom: "0.5rem", color: "#9ca3af" }}>
                        Status
                      </div>
                      <div style={{ fontSize: "1.25rem", fontWeight: 700, color:
                        lineData.loading_pct >= 100 ? "#ef4444" :
                        lineData.loading_pct >= 90 ? "#f97316" :
                        lineData.loading_pct >= 60 ? "#eab308" :
                        "#10b981"
                      }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1.5rem" }}>
                      <div style={{ background: "rgba(255, 255, 255, 0.05)", padding: "1rem", borderRadius: "10px" }}>
                        <div style={{ fontSize: "0.75rem", color: "#9ca3af", marginBottom: "0.5rem" }}>Loading</div>
                        <div style={{ fontSize: "1.75rem", fontWeight: 700 }}>
                          {lineData.loading_pct.toFixed(1)}%
                        </div>
                      </div>
                      <div style={{ background: "rgba(255, 255, 255, 0.05)", padding: "1rem", borderRadius: "10px" }}>
                        <div style={{ fontSize: "0.75rem", color: "#9ca3af", marginBottom: "0.5rem" }}>Rating</div>
                        <div style={{ fontSize: "1.75rem", fontWeight: 700 }}>
                          {lineData.rating_mva.toFixed(1)} <span style={{ fontSize: "1rem", color: "#9ca3af" }}>MVA</span>
                        </div>
                      </div>
                      <div style={{ background: "rgba(255, 255, 255, 0.05)", padding: "1rem", borderRadius: "10px" }}>
                        <div style={{ fontSize: "0.75rem", color: "#9ca3af", marginBottom: "0.5rem" }}>Flow</div>
                        <div style={{ fontSize: "1.75rem", fontWeight: 700 }}>
                          {lineData.flow_mva.toFixed(1)} <span style={{ fontSize: "1rem", color: "#9ca3af" }}>MVA</span>
                        </div>
                      </div>
                      <div style={{ background: "rgba(255, 255, 255, 0.05)", padding: "1rem", borderRadius: "10px" }}>
                        <div style={{ fontSize: "0.75rem", color: "#9ca3af", marginBottom: "0.5rem" }}>Margin</div>
                        <div style={{ fontSize: "1.75rem", fontWeight: 700, color: lineData.margin_mva < 0 ? "#ef4444" : "#10b981" }}>
                          {lineData.margin_mva >= 0 ? "+" : ""}{lineData.margin_mva.toFixed(1)} <span style={{ fontSize: "1rem", color: "#9ca3af" }}>MVA</span>
                        </div>
                      </div>
                    </div>

                    {/* Additional Details */}
                    <div style={{ background: "rgba(255, 255, 255, 0.05)", padding: "1.25rem", borderRadius: "12px" }}>
                      <h3 style={{ fontSize: "1rem", marginBottom: "1rem", fontWeight: 600 }}>Line Details</h3>
                      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          <span style={{ color: "#9ca3af" }}>Conductor:</span>
                          <span style={{ fontWeight: 500 }}>{lineData.conductor || "N/A"}</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          <span style={{ color: "#9ca3af" }}>Max Temperature:</span>
                          <span style={{ fontWeight: 500 }}>{lineData.max_temp?.toFixed(1) || "N/A"}°C</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          <span style={{ color: "#9ca3af" }}>Length:</span>
                          <span style={{ fontWeight: 500 }}>{lineData.length?.toFixed(2) || "N/A"} km</span>
                        </div>
                      </div>
                    </div>
                  </div>
